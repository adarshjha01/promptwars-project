...
import { saveTriageResult, type TriageResultData } from "@/lib/firestore.service";
...
function RiskBadge({ level }: { level: TriageResultData["risk_level"] }) {
...
function RawJsonPanel({ data }: { data: TriageResultData }) {
...
export default function TriageDashboard({ user }: TriageDashboardProps) {
  ...
  const analyzeData = useCallback(async () => {
    ...
    try {
      ...
      const data: TriageResultData = await res.json();
